...
class ProjectDAL(BaseDAL[Project]):
    ...
    async def get_by_organization_id(self, organization_id: str) -> List[Project]:
        ...
        query = (
            select(self.model)
            .where(
                self.model.organization_id == organization_id, ~self.model.is_deleted
            )
    ...
    async def search_by_name(
        self, name_pattern: str, organization_id: Optional[str] = None
    ) -> List[Project]:
    ...
    async def get_by_status(
        self, status: ProjectStatus, organization_id: Optional[str] = None
    ) -> List[Project]:
...
class ApiKeyDAL(BaseDAL[ApiKey]):
    ...
    async def get_by_key_hash(self, key_hash: str) -> Optional[ApiKey]:
        ...
        query = select(self.model).where(
            self.model.key_hash == key_hash, ~self.model.is_deleted
        )
        return await self._get_first(query)
