...
class UserDAL(BaseDAL[User]):
    ...
    async def get_by_email(self, email: str) -> Optional[User]:
        ...
        try:
            return (
                ...
                .filter(and_(self.model.email == email, self.model.is_deleted == False))
                .first()
            )
    ...
    async def get_by_username(self, username: str) -> Optional[User]:
        ...
        try:
            return (
                ...
                .filter(
                    and_(
                        self.model.username == username, self.model.is_deleted == False
                    )
                )
                .first()
            )
    ...
    async def get_by_role(
        ...
        try:
            return (
                self.db.query(self.model)
                .filter(and_(self.model.role == role, self.model.is_deleted == False))
                .offset(skip)
                .limit(limit)
                .all()
            )
    ...
    async def search_users(
        ...
        try:
            ...
            return (
                self.db.query(self.model)
                .filter(and_(search_filter, self.model.is_deleted == False))
                .offset(skip)
                .limit(limit)
                .all()
            )
