...
class OrganizationDAL(BaseDAL[Organization]):
    ...
    async def get_by_name(self, name: str) -> Optional[Organization]:
        ...
        query = select(self.model).where(self.model.name == name, ~self.model.is_deleted)
    ...
    async def search_by_name(self, name_pattern: str) -> List[Organization]:
        ...
        query = select(self.model).where(self.model.name.ilike(f"%{name_pattern}%"), ~self.model.is_deleted).order_by(self.model.name)
    ...
    async def get_total_credits(self) -> int:
        ...
        query = select(func.sum(self.model.centicredits)).where(self.model.status == OrganizationStatus.ACTIVE, ~self.model.is_deleted)
